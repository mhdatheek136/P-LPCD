...
# Ensure the 'images' and 'labels' directories exist with subdirectories for train, val, and test
base_dir = 'images_new'
...
from label_img import create_new_license_plate_bike_front, create_new_license_plate_bike_back
...
def generate_license_plates(num_front, num_back):
    ...
    for i in range(num_front):
        ...
        # 50% chance for city to be "PUNJAB" or " "
        city_text = random.choices(["PUNJAB", " "], weights=[1, 1])[0]

        # Generate the front plate and corresponding label
        label_file = f"labels/lp_front_new_{count_str}.txt"
        output_file = f"images/lp_front_new_{count_str}.jpg"
        create_new_license_plate_bike_front(
            ler_text=ler_text, number_text=number_text, city_text=city_text, 
    ...
    for i in range(num_back):
        ...
        label_file = f"labels/lp_back_new_{count_str}.txt"
        output_file = f"images/lp_back_new_{count_str}.jpg"
        create_new_license_plate_bike_back(
            ler_text=ler_text, number_text=number_text, city_text=city_text,  
            label_file=label_file, 
            output_file=output_file
        )
